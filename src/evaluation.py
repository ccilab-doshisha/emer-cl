...
"""
Compute AE for dimensions larger than corr_p.
...
def diff_upper(
    ...
    corr_idx = np.where(train_corrs >= corr_p)[0]
...
def evaluation(
    ...
    # init
    ae_weighed_corr_m2e, ae_weighed_corr_e2m = 0.0, 0.0
    kl_m2e, kl_e2m = 0.0, 0.0
    ...
    if _lambda != 0:
        ...
        ae_weighed_corr_m2e = diff_upper(
            corr_p, pred_from_line_m2e, f2_hat, train_corrs
        )
        ae_weighed_corr_e2m = diff_upper(
            corr_p, pred_from_line_e2m, f1_hat, train_corrs
        )
